...
describe('Auth Routes', () => {
  ...
  describe('POST /auth', () => {
    it('should generate an auth token and send an email', async () => {
      ...
      expect(generateToken).toHaveBeenCalledWith({ userId: mockUserId, email: mockEmail });
    ...
    it('should return 500 if email sending fails', async () => {
      ...
      expect(response.body).toEqual({ error: 'Failed to send authentication email' });
    });
  });
  ...
  describe('GET /login', () => {
    it('should log in a user and set a JWT cookie', async () => {
      ...
      const response = await request(app).get(`/api/auth/login?token=${mockAuthToken}`);

      expect(response.status).toBe(200);
      expect(response.body).toEqual({ message: 'Logged in successfully' });

      expect(User.findOne).toHaveBeenCalledWith({ where: { authToken: mockAuthToken } });
      expect(generateToken).toHaveBeenCalledWith({ userId: mockUserId, email: mockEmail });
    ...
    it('should return 400 if token is not provided', async () => {
      ...
      expect(response.status).toBe(400);
      expect(response.body).toEqual({ error: 'Token is required and must be a string' });
    });
